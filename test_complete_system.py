    ...
    async def simulate_user_behavior(self, user_id: str, behavior: str):
        ...
        if behavior == "ephemeral":
            ...
            for topic in topics * 10:  # 20 queries
                ...
                profile = self.profile_router._get_profile(user_id)
                if topic not in profile.preferred_topics:
                    profile.preferred_topics.add(topic)
...
async def main():
    ...
    profile_a = nodes[0].profile_router._get_profile("user_a")
    ...
    profile_b = nodes[0].profile_router._get_profile("user_b")
    ...
    profile_d = nodes[0].profile_router._get_profile("user_doanh")
    ...
    for i, node in enumerate(nodes):
        ...
        logger.info(f"  Is Super Node: {node.consensus.is_super_node(node.node_id)}")
    ...
    logger.info(f"\nUser Profiles:")
    logger.info(f"  Total Users: {len(nodes[0].profile_router.user_profiles)}")
    logger.info(f"  VIP Users: {[u for u, p in nodes[0].profile_router.user_profiles.items() if p.tier == UserTier.VIP_PRO]}")
    logger.info(f"  Stable Users: {[u for u, p in nodes[0].profile_router.user_profiles.items() if p.tier == UserTier.STABLE]}")
