...
selected_experts = st.multiselect(
    ...
    help="Choose one or more specialized AI agents to help with your request."
)

user_instruction = st.text_area(
    "Enter your instruction or question:",
    placeholder="e.g., 'Analyze the security implications of quantum computing'",
    help="Be specific to get the best results from the expert network."
)
...
if st.session_state.last_response:
    ...
    st.caption(f"🛡️ Experts consulted: {', '.join([EXPERT_MAP.get(e, e) for e in response_data['experts']])}")
    st.caption(f"🆔 Dispatch ID: {response_data['dispatch_id']}")

    st.write("How would you rate this response?")

    feedback_score = st.slider(
        "Rating (0.0 = Needs Improvement, 1.0 = Excellent)",
        0.0, 1.0, 0.75, 0.05,
        help="Your feedback directly improves the network's reputation and expert selection."
    )
