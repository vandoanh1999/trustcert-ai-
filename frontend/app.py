...
selected_experts = st.multiselect(
    ...
    help="Choose which LoRA adapters to activate for this query."
)

user_instruction = st.text_area(
    "Enter your instruction or question:",
    placeholder="e.g., Explain the implications of decentralized consensus on AI safety...",
...
with col1:
    ...
    if st.button(
        ...
        help="Ready to dispatch!" if not query_disabled else "Please enter an instruction and select at least one expert."
    ):
...
with col2:
    st.button("Clear", on_click=reset_ui, use_container_width=True, help="Reset the input and results.")
...
if st.session_state.last_response:
    ...
    with st.chat_message("🌌"):
        ...
        st.caption(f"Generated by: {', '.join([EXPERT_MAP.get(e, e) for e in response_data['experts']])} | Dispatch ID: {response_data['dispatch_id']}")
