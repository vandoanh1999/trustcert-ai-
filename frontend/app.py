...
available_experts = list(EXPERT_MAP.values())

selected_experts_friendly = st.multiselect(
    "Select Experts to Consult (simulation):",
    options=available_experts,
    default=available_experts[:2],
    help="Choose which specialized AI models should process your request."
)

# Map back to technical paths for the backend
selected_experts = [REVERSE_EXPERT_MAP[name] for name in selected_experts_friendly]

user_instruction = st.text_area(
    "Enter your instruction or question:",
    placeholder="e.g., 'Analyze the current network health and suggest optimizations' or 'Draft a proposal for a new geology expert model'"
)
...
if st.session_state.last_response:
    ...
    # Show friendly names in the caption
    friendly_experts = [EXPERT_MAP.get(e, e) for e in response_data['experts']]
    st.caption(f"Generated by: {', '.join(friendly_experts)}")
    ...
    if st.button("Submit Feedback"):
        ...
        st.toast("Feedback submitted! Thank you for helping Genesis learn. 🚀", icon="✅")
