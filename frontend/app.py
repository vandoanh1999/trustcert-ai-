...
def query_genesis_backend(instruction, experts):
    ...
    try:
        ...
        instr_preview = instruction[:30]
        mock_response_text = (
            f"This is a simulated response for your query about "
            f"'{instr_preview}...' using experts {experts}."
        )
...
with st.sidebar:
    ...
    if st.button(
        "Reset Session",
        help="Clears the current query and feedback state."
    ):
        st.session_state.last_response = None
    ...
    st.divider()
    st.info("Genesis Core V8 - Portal Pillar")
