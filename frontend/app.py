...
def send_feedback_to_backend(dispatch_id, score):
    ...
    try:
        ...
        response = requests.post(feedback_url, json={"score": score}, timeout=5)
        if response.status_code == 200:
            return True, f"Feedback ({score}/1.0) submitted successfully!"
        else:
            return False, f"Failed to submit feedback (Error {response.status_code})."
    except requests.exceptions.RequestException as e:
        return False, f"Error connecting to the backend: {e}"
...
selected_experts = st.multiselect(
    ...
    help="Experts are specialized LoRA models that collaborate to solve your task. Different experts excel at different domains.",
    placeholder="Choose one or more experts..."
)

user_instruction = st.text_area(
    "Enter your instruction or question:",
    placeholder="e.g., 'Analyze the security of this P2P protocol' or 'Summarize the latest network activity'",
    help="Provide a clear and concise instruction for the Genesis network to process."
)
...
if st.session_state.last_response:
    ...
    if st.button("Submit Feedback", type="primary"):
        ...
        success, message = send_feedback_to_backend(response_data["dispatch_id"], feedback_score)
        if success:
            st.toast(message, icon="✅")
            # Clear state and refresh after a short delay for visibility
            st.session_state.last_response = None
            time.sleep(1)
            st.rerun()
        else:
