...
def send_feedback_to_backend(dispatch_id, score):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            time.sleep(1) # Give time for the toast to be seen before rerun
            st.rerun()
        else:
...
user_instruction = st.text_area("Enter your instruction or question:", placeholder="e.g., Explain the P2P gossip protocol...")

# Button is disabled until input is provided, with a helpful tooltip
query_disabled = not user_instruction or not selected_experts
help_text = "Enter an instruction and select at least one expert to enable." if query_disabled else "Dispatch your query to the Genesis network."

if st.button("Query Genesis", disabled=query_disabled, help=help_text):
