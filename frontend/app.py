...
# --- Sidebar ---
with st.sidebar:
    st.title("⚙️ Genesis Portal Settings")
    st.markdown("---")
    if st.button("🗑️ Clear Conversation", help="Reset the current session and clear responses."):
        st.session_state.last_response = None
        st.rerun()
...
user_instruction = st.text_area("Enter your instruction or question:", placeholder="e.g., How does the P2P gossip protocol ensure data integrity?")

# Help text for the disabled button
button_help = ""
if not user_instruction:
    button_help = "Please enter an instruction to query Genesis."
elif not selected_experts:
    button_help = "Please select at least one expert to consult."

if st.button("Query Genesis", disabled=not user_instruction or not selected_experts, help=button_help):
