...
def query_genesis_backend(instruction, experts):
    ...
    try:
        ...
        friendly_experts = [EXPERT_MAP.get(e, e) for e in experts]
        mock_response_text = f"This is a simulated response for your query about '{instruction[:30]}...' using experts {friendly_experts}."
...
available_options = list(EXPERT_MAP.values())
...
user_instruction = st.text_area(
    ...
    placeholder="e.g., 'Analyze the impact of decentralization on AI safety' or 'Write a smart contract for a reputation-based DAO'"
)

if st.button("Query Genesis", disabled=not user_instruction or not selected_friendly):
    # Map friendly names back to technical paths
    REVERSE_MAP = {v: k for k, v in EXPERT_MAP.items()}
    selected_experts = [REVERSE_MAP[name] for name in selected_friendly]
