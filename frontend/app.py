...
# Human-readable mapping for experts
EXPERT_MAP = {
    "dummy_adapters/expert_A/adapter_model.bin": "Research Assistant (Expert A)",
    "dummy_adapters/expert_B/adapter_model.bin": "Creative Writer (Expert B)",
    "dummy_adapters/expert_C/adapter_model.bin": "Data Analyst (Expert C)"
}
...
selected_expert_paths = st.multiselect(
    ...
    help="Choose one or more specialized AI agents to help process your request."
)

user_instruction = st.text_area(
    "Enter your instruction or question:",
    placeholder="e.g., 'Summarize the recent findings in quantum computing...'"
)

if st.button(
    "Query Genesis",
    disabled=not user_instruction or not selected_expert_paths,
    help="Click to send your instruction to the selected expert network."
):
    with st.spinner("Dispatching query to the expert network..."):
        selected_experts = selected_expert_paths
