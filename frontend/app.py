...
def send_feedback_to_backend(dispatch_id, score):
    ...
    try:
        ...
        if response.status_code == 200:
            st.toast(f"Feedback ({score}) submitted! Thanks for helping Genesis grow.", icon="✨")
...
selected_experts = st.multiselect(
    ...
    help="In the live network, Genesis automatically routes your query to the best experts."
)

user_instruction = st.text_area(
    "Enter your instruction or question:",
    placeholder="e.g., Summarize the latest security audit for node_5...",
    help="Be as specific as possible for the best results from the expert network."
)
...
if st.session_state.last_response:
    ...
    # --- Dynamic Sentiment Feedback Logic ---
    sentiment_map = [
        (0.9, "🤩 Exceptional"),
        (0.7, "😊 Great"),
        (0.5, "🙂 Good"),
    ...
    feedback_score = st.slider("Select your rating:", 0.0, 1.0, 0.75, 0.05, help="Your feedback directly affects the reputation of the experts involved.")

    # Visual sentiment reinforcement
    current_sentiment = next(label for score, label in sentiment_map if feedback_score >= score)
    st.info(f"Your Rating: **{feedback_score}** — {current_sentiment}")
