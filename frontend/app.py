...
selected_experts = st.multiselect(
    ...
    help="Choose AI specialists to collaborate on your request."
)

user_instruction = st.text_area(
    "Enter your instruction or question:",
    placeholder="e.g., Explain the importance of zero-knowledge proofs.",
    help="Provide clear instructions for the expert network."
)

if st.button("Query Genesis",
             disabled=not user_instruction or not selected_experts,
             use_container_width=True,
             help="Send your query to the network."):
...
if st.session_state.last_response:
    ...
    with st.chat_message("🌌"):
        ...
        st.caption(f"✨ Collaboration: {', '.join([EXPERT_MAP.get(e, e) for e in response_data['experts']])}")
        st.caption(f"🆔 ID: {response_data['dispatch_id']}")
