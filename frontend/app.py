...
selected_experts = st.multiselect(
    ...
    help="Choose one or more specialized AI agents to handle your request."
)

user_instruction = st.text_area(
    "Enter your instruction or question:",
    placeholder="e.g., 'Analyze the security of this smart contract' or 'Explain the history of the Byzantine Fault Tolerance'",
    help="Provide a clear and detailed prompt for the Genesis network."
)
...
if st.session_state.last_response:
    ...
    with st.chat_message("assistant", avatar="🌌"):
        ...
        st.caption(f"Experts: {', '.join([EXPERT_MAP.get(e, e) for e in response_data['experts']])} | ID: {response_data['dispatch_id']}")
    ...
    feedback_score = st.slider(
        ...
        value=0.75,
        step=0.05,
        help="Slide to rate the quality of the response. 1.0 is a perfect answer."
    )
    col1, col2, col3 = st.columns([1, 1, 1])
    col1.caption("🌑 Disappointing")
    col2.markdown("<center>🌓 Satisfactory</center>", unsafe_allow_html=True)
    col3.markdown("<p style='text-align: right;'>🌕 Exceptional</p>", unsafe_allow_html=True)
