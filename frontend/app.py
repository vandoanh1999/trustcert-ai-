...
def send_feedback_to_backend(dispatch_id, score):
    ...
    try:
        ...
        if response.status_code == 200:
            st.toast(f"Feedback ({score}/1.0) submitted! 🌌", icon="🌌")
            # Clear the last response to be ready for the next query
            st.session_state.last_response = None
            time.sleep(1) # Brief pause for toast to be seen
            st.rerun()
        else:
...
with st.sidebar:
    ...
    if st.button("🗑️ Clear Conversation", use_container_width=True):
        ...
        st.rerun()
    st.divider()
    st.info("Genesis Hub connects you to a decentralized network of AI experts.")
...
selected_experts = st.multiselect(
    ...
    help="Choose which AI experts should process your request."
)

user_instruction = st.text_area(
    "Enter your instruction or question:",
    placeholder="e.g., How does the P2P gossip protocol work in Genesis?",
    help="Type your query here. The network will route it to the best available nodes."
)

if st.button(
    "Query Genesis",
    disabled=not user_instruction or not selected_experts,
    help="Submit your query to the decentralized expert network." if user_instruction and selected_experts else "Please enter a query and select experts first."
):
...
if st.session_state.last_response:
    st.divider()
    st.header("2. Genesis Response")

    response_data = st.session_state.last_response

    with st.chat_message("🌌"):
        st.markdown(response_data["text"])
        st.caption(f"**Experts:** {', '.join([EXPERT_MAP.get(e, e) for e in response_data['experts']])}")
        st.caption(f"**Dispatch ID:** `{response_data['dispatch_id']}`")

    st.divider()
    st.header("3. Provide Feedback")
    st.write("Your feedback helps improve the network's reputation system.")

    feedback_score = st.slider("Rate the response accuracy:", 0.0, 1.0, 0.75, 0.05)
