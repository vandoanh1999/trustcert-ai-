...
user_instruction = st.text_area(
    ...
    placeholder="e.g., How can I optimize my local expert network for low-latency inference?",
    help="Provide a clear instruction or question for the Genesis expert network to process."
)

if st.button("✨ Query Genesis", disabled=not user_instruction or not selected_experts, type="primary", use_container_width=True):
    with st.status("Dispatching query to the expert network...", expanded=True) as status:
        st.write("🔍 Identifying suitable experts...")
        dispatch_id, response_text = query_genesis_backend(user_instruction, selected_experts)
        if dispatch_id:
            st.write("✅ Response received from network.")
...
if st.session_state.last_response:
    ...
    with st.chat_message("🌌"):
        st.write("**Genesis Network Response:**")
