...
def query_genesis_backend(instruction, experts):
    ...
    st.info(f"Querying with experts: {', '.join(friendly_experts)}...")
...
def reset_ui():
    ...
    st.session_state.last_response = None
    # We use keys for text area and multiselect to reset them
    if "instruction_input" in st.session_state:
        st.session_state.instruction_input = ""
    if "expert_selection" in st.session_state:
...
with col1:
    query_button = st.button(
        ...
        help="Click to dispatch your query to the decentralized expert network."
    )
with col2:
    st.button("Clear", on_click=reset_ui, help="Clear the current query and response.")
