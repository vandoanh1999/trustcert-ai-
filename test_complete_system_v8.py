...
# --- Mocks for testing if files have descriptive suffixes ---
try:
    from core.consensus import ProofOfContribution
except ImportError:
    import core.consensus as consensus_mod
    ProofOfContribution = consensus_mod.ProofOfContribution

try:
    from core.secure_dtq import MPCDistributedTaskQueue
except ImportError:
    # Handle the file with spaces/suffixes if necessary, but here we use the name we found
    pass
...
class CompleteTestNode:
    ...
    def __init__(self, node_id: str, port: int):
        ...
        self.dtq.consensus = self.consensus
        ...
        # Register handlers
        self.p2p.external_handlers.append(self.consensus.handle_peer_message)
        self.p2p.external_handlers.append(self.dtq.handle_p2p_message)
        self.dtq.handlers['test_task'] = self.handle_test_task
...
async def main():
    ...
    # Manually ensure we have super nodes for the task
    for node in nodes:
        node.dtq.p2p.peers.update([f"127.0.0.1:{8765+i}" for i in range(5) if i != int(node.node_id.split('_')[1])])
