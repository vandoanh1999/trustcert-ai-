...
class FaissVectorStore:
    ...
    def save(self, text: str, embedding: np.ndarray, 
        ...
        # BOLT OPTIMIZATION: O(1) duplicate check using in-memory set
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        # BOLT OPTIMIZATION: Fetch metadata in a single batch query instead of N individual queries (Fixes N+1)
        ...
        for score, idx in zip(scores[0], indices[0]):
            idx_int = int(idx)
            if idx == -1 or idx_int not in metadata_map:
                continue
