...
class FaissVectorStore:
    ...
    def __init__(self, node_id: str, dimension: int = 384, 
        ...
        self.vector_ids_set = set(self.vector_ids) # BOLT OPTIMIZATION: O(1) duplicate check
    ...
    def save(self, text: str, embedding: np.ndarray, 
        ...
        # Check duplicate
        if vec_id in self.vector_ids_set: # BOLT OPTIMIZATION: use set for O(1) lookup
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        # Fetch metadata
        # BOLT OPTIMIZATION: Batch fetch metadata with a single SQL query instead of N queries
        valid_indices = [int(idx) for idx in indices[0] if idx != -1]
        if not valid_indices:
            return []

        placeholders = ','.join(['?'] * len(valid_indices))
        query_str = f"SELECT idx, id, text, metadata FROM vectors WHERE idx IN ({placeholders})"
        cursor = self.conn.execute(query_str, valid_indices)

        metadata_map = {row[0]: (row[1], row[2], row[3]) for row in cursor.fetchall()}

        results = []
        for score, idx in zip(scores[0], indices[0]):
            if idx != -1 and int(idx) in metadata_map:
                m = metadata_map[int(idx)]
                results.append({
                    "id": m[0],
                    "text": m[1],
                    "metadata": m[2],
