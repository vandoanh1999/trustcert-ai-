...
class FaissVectorStore:
    ...
    def save(self, text: str, embedding: np.ndarray, 
        ...
        # BOLT OPTIMIZATION: Use set for O(1) duplicate check (was O(N) list scan)
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        # BOLT OPTIMIZATION: Batched metadata retrieval to avoid N+1 queries
        valid_indices = [int(idx) for idx in indices[0] if idx != -1]
        if not valid_indices:
            return []

        # Fetch all matching metadata in one query
        ...
        # Map results by idx for fast lookup while maintaining score order
