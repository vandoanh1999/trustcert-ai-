...
class FaissVectorStore:
    ...
    def __init__(self, node_id: str, dimension: int = 384, 
        ...
        # BOLT OPTIMIZATION: Use a set for O(1) duplicate checks in save()
    ...
    def save(self, text: str, embedding: np.ndarray, 
        ...
        # Check duplicate
        # BOLT OPTIMIZATION: Use O(1) set lookup instead of O(N) list lookup
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        # BOLT OPTIMIZATION: Batch SQLite metadata lookups to reduce roundtrips
        valid_indices = [int(idx) for idx in indices[0] if idx != -1]
        if not valid_indices:
            return []

        # Create a mapping for quick lookup after batch fetch
