...
class FaissVectorStore:
    ...
    def save(self, text: str, embedding: np.ndarray, 
        ...
        # BOLT OPTIMIZATION: Batch commits to reduce disk I/O
        # Persist index and commit DB every 100 vectors
        if idx % 100 == 0:
            self.conn.commit()
            self._persist_index()
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        # BOLT OPTIMIZATION: Batched SQL query to eliminate N+1 problem
        # Fetch metadata for all indices in a single query
        target_indices = [int(idx) for idx in indices[0] if idx != -1]
        if not target_indices:
            return []

        placeholders = ",".join(["?"] * len(target_indices))
        query_sql = f"SELECT idx, id, text, metadata FROM vectors WHERE idx IN ({placeholders})"
        cursor = self.conn.execute(query_sql, target_indices)

        # Map rows by idx for fast lookup while maintaining FAISS score order
