...
class FaissVectorStore:
    ...
    def __init__(self, node_id: str, dimension: int = 384, 
        ...
        # Vector ID mapping
        # BOLT OPTIMIZATION: Use a set for O(1) duplicate checks and a list for ordered access
    ...
    def vector_ids(self) -> List[str]:
        """Maintain backward compatibility for ordered vector ID access"""
    ...
    def save(self, text: str, embedding: np.ndarray, 
        ...
        # Check duplicate
        # BOLT OPTIMIZATION: O(1) membership check using a set instead of O(N) list search
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        # Fetch metadata
        # BOLT OPTIMIZATION: Fix N+1 query problem by fetching all metadata in a single batch
