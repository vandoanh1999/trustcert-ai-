...
class FaissVectorStore:
    ...
    def save(self, text: str, embedding: np.ndarray, 
        ...
        # BOLT OPTIMIZATION: Use set for O(1) duplicate check instead of O(N) list search
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        # BOLT OPTIMIZATION: Fetch all metadata in a single SQL query using IN clause
        # This avoids N+1 query problem and multiple roundtrips to SQLite
        ...
        for score, idx in zip(scores[0], indices[0]):
            if idx == -1 or int(idx) not in metadata_map:
                continue
            
            row = metadata_map[int(idx)]
            results.append({
                "id": row[0],
                "text": row[1],
                "metadata": row[2],
