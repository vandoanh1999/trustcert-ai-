...
class FaissVectorStore:
    ...
    def search(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        scores, indices = self.index.search(
            query.reshape(1, -1), min(top_k, self.index.ntotal)
        )
        ...
        # BOLT OPTIMIZATION: Use a single batch query instead of N individual queries to fix N+1 problem
        ...
        # Map rows by their idx for quick re-ordering
        row_map = {row[0]: row for row in cursor.fetchall()}

        results = []
        for i, idx in enumerate(indices[0]):
            if idx == -1 or idx not in row_map:
                continue
