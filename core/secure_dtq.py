...
class MPCDistributedTaskQueue:
    ...
    async def _get_super_nodes(self) -> List[str]:
        ...
        # We need a reference to consensus or a way to get it
        # For now, let's assume it's available via p2p or similar if we can find it
        # In test_complete_system_v8.py, CompleteTestNode has both.
        # Let's add consensus to MPCDistributedTaskQueue
    ...
    async def handle_key_share_distribute(self, task_id: str, share_hex: str):
        """Store key share distributed by task creator"""
        self.my_key_shares[task_id] = bytes.fromhex(share_hex)
        logger.info(f"📥 Received and stored key share for task: {task_id}")

    async def handle_p2p_message(self, message: Dict):
        """P2P message dispatcher for DTQ"""
        ...
        if msg_type == 'secure_task_announce':
            ...
            await self.handle_key_share_distribute(
                message['task_id'],
                message['share']
            )

        elif msg_type == 'key_share_request':
            await self.handle_key_share_request(
                message['requester'],
                message['task_id']
            )
