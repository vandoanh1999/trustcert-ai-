...
class MPCDistributedTaskQueue:
    ...
    async def submit_secure_task(self, task_type: str, payload: dict,
        ...
        # 6. Distribute key shares to peers (simplified: just first few peers)
        peers = list(self.p2p.peers)
        for i, peer in enumerate(peers[:num_shares]):
            await self.p2p.send_to_peer(peer, {
