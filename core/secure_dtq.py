...
class MPCDistributedTaskQueue:
    ...
    async def start_worker(self):
        """Worker loop to process pending tasks"""
            ...
            try:
                ...
                for task_id, task in self.tasks.items():
                    if task['status'] == 'pending' and task_id not in self.running_tasks:
                        # Simplified logic: every node tries to claim/execute
    ...
    async def _get_super_nodes(self) -> List[str]:
        """Get super nodes from consensus"""
        # We need to access consensus from somewhere. In this architecture,
        # usually nodes have a reference to all components.
        # For simplicity, we assume the p2p network might have access to it
        # or we find another way. In test_complete_system, the node has it.
        # But DTQ only has p2p.

        # Let's try to find it via p2p if possible, or just ask peers
        response = await self.p2p.send_and_wait(self.node_id, {"type": "get_super_nodes"})
        if response and response.get('super_nodes'):
            return response['super_nodes']
        return []