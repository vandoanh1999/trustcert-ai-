...
class GossipP2P:
    ...
    async def broadcast(self, message: Dict):
        ...
        if not peers:
            return
        await asyncio.gather(*[self._safe_send(p, message) for p in peers])
    ...
    async def query_peers(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        # Parallel query with return_exceptions=True to avoid one failure stopping all
        responses = await asyncio.gather(
            *[self.send_and_wait(p, message) for p in peers],
