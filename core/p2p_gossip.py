...
class GossipP2P:
    ...
    async def _safe_send(self, peer: str, message: Dict):
        """Helper to send to peer without raising exception for broadcast"""
        try:
    ...
    async def broadcast(self, message: Dict):
        ...
        # Parallelize broadcast for performance ⚡
        tasks = [self._safe_send(peer, message) for peer in list(self.peers)]
    ...
    async def query_peers(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        # Parallelize peer queries for performance ⚡
