...
class GossipP2P:
    ...
    async def broadcast(self, message: Dict):
        ...
        async def _safe_send(peer: str):
            try:
                await self.send_to_peer(peer, message)
            except Exception as e:
                logger.warning(f"Broadcast to {peer} failed: {e}")

        # Parallelize broadcasting to all peers
        await asyncio.gather(*[_safe_send(peer) for peer in list(self.peers)])
    ...
    async def query_peers(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict]:
        ...
        async def _safe_query(peer: str):
            try:
                response = await self.send_and_wait(peer, message)
                if response and response.get('type') == 'query_response':
                    return response['results']
            except Exception as e:
                logger.debug(f"Query to {peer} failed: {e}")
            return []

        # Parallelize queries to all peers
        results_list = await asyncio.gather(*[_safe_query(peer) for peer in list(self.peers)])
